...
import { GET_CLIENTS, ADD_CLIENTS, DELETE_CLIENTS, UPDATE_CLIENTS } from 'API/Urls';
...
//Add Leads Api
const addClients = createAsyncThunk('clients/addClients', async (clientData) => {
  const userData = JSON.parse(localStorage.getItem('userData'));
  try {
...
// Update Leads:
const updateClients = createAsyncThunk('clients/updateClients', async ({ value, clientId }) => {
  const userData = JSON.parse(localStorage.getItem('userData'));
  try {
    const response = await axios.patch(UPDATE_CLIENTS + `/${clientId}`, value, {
      headers: {
        Authorization: `Bearer ${userData.accesstoken}`
      }
    });
