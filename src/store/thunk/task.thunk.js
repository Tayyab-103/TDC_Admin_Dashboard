...
import { GET_TASK, ADD_TASK, DELETE_TASK, UPDATE_TASK } from 'API/Urls';
...
//Add Leads Api
const addTask = createAsyncThunk('task/addTask', async (taskData) => {
  const userData = JSON.parse(localStorage.getItem('userData'));
  try {
...
// Update Leads:
const updateTask = createAsyncThunk('task/updateTask', async ({ value, taskId }) => {
  const userData = JSON.parse(localStorage.getItem('userData'));
  try {
    const response = await axios.patch(UPDATE_TASK + `/${taskId}`, value, {
      headers: {
        Authorization: `Bearer ${userData.accesstoken}`
      }
    });
