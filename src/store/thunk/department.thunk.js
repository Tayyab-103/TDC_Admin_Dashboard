...
//Get All Departments:
const getDepartments = createAsyncThunk('departments/getDepartments', async () => {
...
const addDepartments = createAsyncThunk('departments/addDepartments', async (value) => {
...
const deleteDepartments = createAsyncThunk('departments/deleteDepartments', async (id) => {
  const userData = JSON.parse(localStorage.getItem('userData'));
  try {
...
// Update departments:
const updateDepartments = createAsyncThunk('departments/updateDepartments', async ({ value, departmentId }) => {
  // console.log("Id", id);
  const userData = JSON.parse(localStorage.getItem('userData'));
  try {
    const response = await axios.patch(UPDATE_DEPARTMENTS + `/${departmentId}`, value, {
      headers: {
        Authorization: `Bearer ${userData.accesstoken}`
      }
    });
