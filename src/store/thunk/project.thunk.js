...
export const addProject = createAsyncThunk('data/addProjects', async ({ projectData }) => {
  const userData = JSON.parse(localStorage.getItem('userData'));
  try {
...
export const editProject = createAsyncThunk('data/editProjects', async (projectData) => {
  const userData = JSON.parse(localStorage.getItem('userData'));
  try {
    const response = await axios.patch(`${ProjectsUrl}/${projectData._id}`, projectData, {
      headers: {
        Authorization: `Bearer ${userData.accesstoken}`
      }
    });
