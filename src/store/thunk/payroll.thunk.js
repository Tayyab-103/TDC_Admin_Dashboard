...
import { ADD_PAYROLL, DELETE_PAYROLL, GET_PAYROLL, UPDATE_PAYROLL } from 'API/Urls';
...
// Update PayRoll:
const updatePayRoll = createAsyncThunk('payroll/updatePayRoll', async ({ value, payrollId }) => {
  const userData = JSON.parse(localStorage.getItem('userData'));
  try {
    const response = await axios.put(UPDATE_PAYROLL + `/${payrollId}`, value, {
      headers: {
        Authorization: `Bearer ${userData.accesstoken}`
      }
    });
