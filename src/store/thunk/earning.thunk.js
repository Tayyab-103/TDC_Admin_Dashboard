...
import { ADD_EARNING, DELETE_EARNING, GET_EARNING, UPDATE_EARNING } from 'API/Urls';
...
// Update Earning:
const updateEarning = createAsyncThunk('earning/updateEarning', async ({ value, earningId }) => {
  const userData = JSON.parse(localStorage.getItem('userData'));
  try {
    const response = await axios.patch(UPDATE_EARNING + `/${earningId}`, value, {
      headers: {
        Authorization: `Bearer ${userData.accesstoken}`
      }
    });
